...
function getPatientByPage(dataSearch){
    $.ajax({
        type: "POST",
        url: "/api/patient/getPatientList" ,
        dataType: 'json',
        data: dataSearch,
        headers: {"token": localStorage.getItem('token'),"userid": localStorage.getItem('userid')},
        success: function (response) {
            $('.data-table-profile-patient').find('tbody').empty();
            ...
            if(response.responseCode == '00000'){
                ...
                var data = response.data.patientDetailDtos;
                if(data != null || data != undefined){
                    ...
                    for(var i = 0; i < data.length; i++){
                        ...
                        html += '<td><button type="button" class="btn btn-block btn-outline-info btn-xs" ';
                        html += ' onclick="viewProfilePatient(' + data[i].code + ')"' + '><span class="bi bi-eye"></span></button>';
                        html += '</td>';
                        html += '</tr>';
                    }
                }
                $('.data-table-profile-patient').find('tbody').html(html);
                eventPagingUser();
            }
        },
...
function viewProfilePatient(id){
    location.href = '/patient/patient-detail?id=' + id;
}