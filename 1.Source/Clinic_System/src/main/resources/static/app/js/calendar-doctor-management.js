...
function validateAddCalender(){
    $('.error-app').remove();
    var check = true;
    var today = new Date();

    if($('#booking-date-admission-add').val().trim() == ''){
            $('#booking-date-admission-add').parent().after( $( '<p class="error-app" style="color: red;font-size: 14px;">Ngày trực không để trống</p>' ) );
            check = false;
        }else{
            if(moment($('#booking-date-admission-add').val().trim(), 'DD/MM/YYYY').toDate() < moment(moment().format('DD/MM/YYYY'), 'DD/MM/YYYY').toDate()){
                $('#booking-date-admission-add').parent().after( $( '<p class="error-app" style="color: red;font-size: 14px;">Ngày trực không được nhỏ hơn ngày hiện tại</p>' ) );
                check = false;
            }
            if(isNaN($('#booking-date-admission-add').val().replaceAll('/',''))){
                $('#booking-date-admission-add').parent().after( $( '<p class="error-app" style="color: red;font-size: 14px;">Ngày trực không để trống</p>' ) );
                check = false;
            }
        }
