...
function loadAllActionDoctor(){
    ...
    $('.action-examining').unbind().click(function(){
        ...
        if(location.search.split('id=').length > 1){
            id = location.search.split('id=')[1];
            location.href = '/doctor/list-patient-management';
        }
...
function validate(){
    ...
    var check = true;
    var numberRegex = /^\d+$/;
    var temperatureRegex = /^((3[5-9]|4[0-1])((.[0-9])?))|(42((.[0])?))$/;
    if($('#temperature-patient').val().trim() !== ''){
        if(temperatureRegex.test($('#temperature-patient').val()) == false){
            $('#temperature-patient').parent().after( $( '<div class="error-user" style="color: red;font-size: 14px;">Nhiệt độ trong khoảng 35.0 - 42.0 độ</div>' ) );
            check = false;
        }
    }else{
