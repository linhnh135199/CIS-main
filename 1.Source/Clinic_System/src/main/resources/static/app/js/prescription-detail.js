...
function validateThuoc(){
    $('.error-user').remove();
    var check = true;
    if($(".so-luong-prescription-text").val() !== ''){
        if(Number($(".so-luong-prescription-text").val()) <= 0){
            check = false;
            $(".so-luong-prescription-text").parent().after( $( '<p class="error-user" style="color: red;font-size: 14px;">Số lượng không nhở hơn 0</p>' ) );
        }
    }else{
        check = false;
        $(".so-luong-prescription-text").parent().after( $( '<p class="error-user" style="color: red;font-size: 14px;">Số lượng không để trống</p>' ) );
    }
    return check;
}
