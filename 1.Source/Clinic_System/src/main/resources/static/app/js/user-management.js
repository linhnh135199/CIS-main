...
function getUserByPage(dataSearch){
    if(location.href.split('/user/doctor-management').length > 1){
        $('.code-user').text('Mã bác sỹ');
        $('.title-user-list').text('Danh sách bác sỹ');
    } else if(location.href.split('/user/patient-management').length > 1){
        $('.code-user').text('Mã bệnh nhân');
        $('.title-user-list').text('Danh sách bệnh nhân');
        $('.bg-gradient-primary').hide();
    } else if(location.href.split('/user/receptionist-management').length > 1){
        $('.code-user').text('Mã lễ tân');
        $('.title-user-list').text('Danh sách lễ tân');
    } else if(location.href.split('/user/admin-management').length > 1){
        $('.code-user').text('Mã quản lý');
        $('.title-user-list').text('Danh sách quản lý');
    }
    $.ajax({
        type: "GET",
...
//    }
    if($('#birthday-user').val().trim() == ''){
        ...
        if( moment($('#birthday-user').val().trim(), 'DD/MM/YYYY').toDate() > moment(moment().format('DD/MM/YYYY'), 'DD/MM/YYYY').toDate()){
